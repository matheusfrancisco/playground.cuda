...
__global__ void pInt(Body *p, float dt, int nBodies)
{
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
    
    for (int i = index; i < nBodies; i+= stride) { // integrate position
      p[i].x += p[i].vx*dt;
      p[i].y += p[i].vy*dt;
      p[i].z += p[i].vz*dt;
    }
...
int main(const int argc, const char** argv) {
   ...
   */
    ...
    pInt<<<numberOfBlocks, threadsPerBlock>>>(p, dt, nBodies);
